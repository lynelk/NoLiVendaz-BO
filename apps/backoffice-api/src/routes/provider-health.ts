...
export async function registerProviderHealthRoutes(app:FastifyInstance):Promise<void>{
 ...
 app.post('/api/v1/connectors/:connectorId/health-check',{preHandler:[app.authenticate,requirePermission('provider.health.check')]},async(request,reply)=>{const {connectorId}=request.params as {connectorId:string};const body=(request.body??{}) as {providerId?:string};if(!body.providerId)return reply.code(400).send({error:'PROVIDER_ID_REQUIRED'});try{const runtime=await getConnectorRuntime(request.principal!,body.providerId,connectorId);if(!runtime.connector.enabled||runtime.connector.status==='DISABLED'||runtime.connector.status==='MAINTENANCE')return reply.code(409).send({error:'CONNECTOR_HEALTH_CHECK_NOT_ALLOWED',message:`Connector is ${runtime.connector.status}`});const healthConnector=runtime.connector.status==='OUTAGE'?{...runtime.connector,status:'DEGRADED' as const}:runtime.connector;const adapter=createProviderAdapter(runtime.providerType,healthConnector,secrets);const health=await adapter.healthCheck();await recordProviderHealth(request.principal!,runtime,health);return{data:{providerId:runtime.providerId,connectorId,...health}};}catch(error){return reply.code(400).send({error:'HEALTH_CHECK_FAILED',message:error instanceof Error?error.message:'Unknown error'});}});
}
