...
function toRuntime(row: Record<string, unknown>): ConnectorRuntimeRecord {
  const connector: ConnectorRecord = {
    id: String(row.id),
    providerId: String(row.providerId2),
...
const selectRuntime = `SELECT
  p.id AS "providerId",
  p.code AS "providerCode",
  p.provider_type AS "providerType",
  pc.id,
  pc.provider_id AS "providerId2",
  pc.name,
  pc.environment,
...
export async function getConnectorRuntime(
  principal: Principal,
  providerId: string,
  connectorId?: string
): Promise<ConnectorRuntimeRecord> {
  return withTenantContext(context(principal), async (client) => {
    const result = await client.query(
      `${selectRuntime}
       WHERE p.id = $1
         AND ($2::uuid IS NULL OR pc.id = $2)
         AND pc.enabled = true
       ORDER BY CASE pc.environment WHEN 'PRODUCTION' THEN 0 ELSE 1 END,
                pc.created_at DESC
       LIMIT 1`,
      [providerId, connectorId ?? null]
    );
    const row = result.rows[0] as Record<string, unknown> | undefined;
    if (!row) throw new Error("ACTIVE_CONNECTOR_NOT_FOUND");
    return toRuntime(row);
  });
}

export async function getConnectorRuntimeForCapability(
  principal: Principal,
  providerId: string,
  ...
  return withTenantContext(context(principal), async (client) => {
    const result = await client.query(
      `${selectRuntime}
       JOIN connector_capabilities cc ON cc.connector_id = pc.id AND cc.enabled = true
       JOIN capabilities cap ON cap.id = cc.capability_id
       WHERE p.id = $1
         AND pc.enabled = true
         AND pc.status IN ('ACTIVE','DEGRADED')
         AND pc.environment = $2
         AND cap.code = $3
       ORDER BY CASE pc.status WHEN 'ACTIVE' THEN 0 ELSE 1 END,
                pc.created_at DESC
       LIMIT 1`,
      [providerId, environment, capabilityCode]
    );
    const row = result.rows[0] as Record<string, unknown> | undefined;
    if (!row) {
      throw new Error(`ACTIVE_CONNECTOR_WITH_CAPABILITY_NOT_FOUND:${capabilityCode}`);
    }
    return toRuntime(row);
  });
}

export async function connectorHasCapability(
  principal: Principal,
  connectorId: string,
  capabilityCode: string
): Promise<boolean> {
  ...
  await withTenantContext(context(principal), async (client) => {
    await client.query(
      `INSERT INTO provider_health_events (
         tenant_id,provider_id,connector_id,health_status,latency_ms,details,checked_at
...
export async function getLatestProviderHealth(
  principal: Principal,
  providerId: string
) {
  return withTenantContext(context(principal), async (client) => {
    const result = await client.query(
      `SELECT
         phe.id,
         phe.provider_id AS "providerId",
         phe.connector_id AS "connectorId",
         phe.health_status AS status,
         phe.latency_ms AS "latencyMs",
         phe.details,
         phe.checked_at AS "checkedAt"
       FROM provider_health_events phe
       WHERE phe.provider_id = $1
       ORDER BY phe.checked_at DESC
       LIMIT 20`,
      [providerId]
    );
    return result.rows;
