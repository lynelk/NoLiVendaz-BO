...
export async function provisionUser(p:Principal,input:{email:string;displayName?:string;isPlatformAdmin?:boolean}){
...
export async function createTenantRole(p:Principal,input:{code:string;name:string;description?:string;permissions:string[]}){
 return withTenantContext(context(p),async client=>{const code=input.code.trim().toUpperCase();const role=(await client.query(`INSERT INTO roles(tenant_id,code,name,description,system_defined) VALUES($1,$2,$3,$4,false) ON CONFLICT(tenant_id,code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description RETURNING id,code,name,description,system_defined`,[p.tenantId,code,input.name,input.description??null])).rows[0];if(!role)throw new Error("ROLE_CREATE_FAILED");const permissions=[...new Set(input.permissions)];if(permissions.length){const resolved=await client.query<{id:string;code:string}>(`SELECT id,code FROM permissions WHERE code=ANY($1::text[])`,[permissions]);if(resolved.rows.length!==permissions.length)throw new Error("UNKNOWN_PERMISSION_IN_ROLE");await client.query(`DELETE FROM role_permissions WHERE role_id=$1`,[role.id]);for(const permission of resolved.rows)await client.query(`INSERT INTO role_permissions(role_id,permission_id) VALUES($1,$2) ON CONFLICT DO NOTHING`,[role.id,permission.id]);}await audit(client,p,"admin.role.upsert","role",String(role.id),{...role,permissions});return{...role,permissions};});
}
