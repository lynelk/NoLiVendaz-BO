...
export async function registerObservability(app:FastifyInstance):Promise<void>{
 ...
 app.get('/ready',async(_r,reply)=>{try{const result=await pool.query<{ready:boolean}>(`SELECT to_regclass('public.transactions') IS NOT NULL AND to_regclass('public.providers') IS NOT NULL AND to_regclass('public.alerts') IS NOT NULL AND to_regclass('public.incidents') IS NOT NULL AS ready`);if(result.rows[0]?.ready)return{status:'ready',service:'backoffice-api'};return reply.code(503).send({status:'not-ready',service:'backoffice-api',reason:'schema-incomplete'});}catch{return reply.code(503).send({status:'not-ready',service:'backoffice-api',reason:'database-unavailable'})}});
